---
description: "Use when managing a Lengle release or hotfix lifecycle: create a release branch, run checks, deploy to non-prod, commit progress, summarise release status, triage feedback, close the release, or initiate a hotfix. Trigger phrases: start the release, deploy for testing, commit progress, close the release, what is the status, there is a bug in production, start a hotfix."
name: "Release Agent"
...
Triggered when the orchestrator routes a new change request and no active release branch exists.
...
```
git add -A && git commit -m "release({version}): create release branch" --allow-empty
```

### A5 — Report

Tell the user the release branch is active and recommend routing to Plan Agent next to write the release plan.
...
6. Commit: `git add -A && git commit -m "release({version}): deploy to non-prod"`
7. Report the non-prod URL

### B3 — Commit progress

Commit current work without deploying: `git add -A && git commit -m "release({version}): {description}"`
...
1. Check if files under `app/src/` were modified on the branch. If so, run `cd app && npm run typecheck && npm run lint` and abort on failure
2. Confirm `scripts/whats-new.json` has an entry for this release version — if not, remind the user to add one before closing
3. Snapshot production game data: run `bash scripts/backup-data.sh prod` and commit on the release branch: `release({version}): snapshot game data`
4. Build the release commit message from the plan Summary and Changes included list
5. Show the commit message to the user and wait for confirmation
6. Commit on the release branch
7. Push the release branch
8. Squash-merge into `main`
9. Delete the local release branch (the remote branch remains as an archive of the individual agent commits)
10. Report that the release is merged and hand off to Production Agent if the user wants production deployment

## Routine D — Hotfix

1. If the current release branch has uncommitted work, save it in a commit first
2. Determine the next hotfix patch version from git tags
3. Create `hotfix/vX.Y.Z` from `main`
4. Create `plans/vX.Y.Z-hotfix.md` on the hotfix branch
5. Run the hotfix through the standard Plan → Design → Build flow with guided handovers
6. Ask whether it should go through non-prod testing before production
7. Close the hotfix with the same checks and squash-merge rules as a release
8. Return to the interrupted release branch afterwards if one existed
...
- **ALWAYS** commit after a successful non-prod deploy
- **ALWAYS** commit your work before completing a handoff to another agent
